...
import torch
from torch.nn import Module, ModuleList, Linear, Dropout, LayerNorm, Identity, Parameter, init, Conv2d, Sequential, ReLU
...
class Attention(Module):
    ...
    def forward(self, x):
        ...
        x = (attn @ v).transpose(1, 2).reshape(B, N, C)  # x: (B, HW + 1, C)

        ## addcode-start ##

        v_t = v.transpose(-1, -2).flatten(1, 2)
        clf_token = v_t[:, :, 0]
    ...
    def get_weight_matrix(H, W, batch_size, channel, dw_kernel):
